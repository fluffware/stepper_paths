...
pub fn find_root(a:i64, b:i64, c: i64, min: i64, max: i64) -> Option<i64>
{
    ...
    if miny == 0 {return Some(minx)};
    if maxy == 0 {return Some(maxx)};
    if same_sign(miny, maxy) {return None;}
    loop {
        if miny == 0 {return Some(minx)};
        if maxy == 0 {return Some(maxx)};
        if maxx - minx <= 1 {
...
{
    let s = b*b + 4*a*c;
    if s < 0 {return None;} // No solution
    // Find minima
    let xmin =
    ...
    if  ymin >= 0 { // Both roots are between two adjacent integers
        if - b <  2 * a * xmin { 
