...
fn main() {
    ...
    let mut ax_max = (ax_scale * config.stepper_x.max_acceleration).abs().round() as i32;
    let mut ay_max = (ay_scale * config.stepper_y.max_acceleration).abs().round() as i32;
    ...
    let filter: Box<svg_parser::ElementFilter> = match matches.opt_str("layer") {
        Some(layer_name) => Box::new(move |name, attrs| {
            match name {
                OwnedName {
                    local_name,
                    ...
                    for a in attrs {
                        ...
                        } = a.name
                        {
                            if namespace == INKSCAPE_NS {
                                if local_name == "groupmode" && a.value == "layer" {
                                    is_layer = true;
                                }
                                if local_name == "label" && a.value == layer_name {
