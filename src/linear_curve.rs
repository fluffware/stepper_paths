...
fn fmt_node(node: &CurveNode, 
            f: &mut Formatter, 
            ind: u32) -> Result<(), Error>
{
    indent(f, ind)?;
...
impl CurveNode {
    fn new(start:f64, end:f64, len:f64) -> CurveNode {
        CurveNode{t_start: start, t_end: end, 
                  length: len, 
                  children: None}
    }
}
...
fn node_length(curve: &dyn CurveApprox, node: &mut Box<CurveNode>) -> f64
{
    let (low_length, high_length) =
        match &node.children {
            None => {
                let (split, low_len, high_len) = 
                    curve.split_length(node.t_start, node.t_end);
                node.children = 
                    Some(CurveNodeChildren{
                        low: Box::new(CurveNode::new(node.t_start, split, 
                                                     low_len.unwrap_or(0.0))),
                        high: Box::new(CurveNode::new(split, node.t_end,
                                                      high_len.unwrap_or(0.0)))
                    });
                (low_len, high_len)
            },
            Some(CurveNodeChildren{low,high}) => {
                (Some(low.length), Some(high.length))
            }
        };
    let children = node.children.as_mut().unwrap();
     node.length = low_length.unwrap_or_else(|| {
...
impl LinearCurve
{
    ...
    {
        let mut top_node = Box::new(CurveNode{t_start: start, t_end: end,
                                          length: 0.0, 
                                          children: None});
        node_length(curve.as_ref(), &mut top_node);
        LinearCurve{curve, node: top_node}
    }
...
impl CurveApprox for TestCurve
{
                    ...
                    -> (f64, Option<f64>, Option<f64>)
    {
        ...
        (start*(1.0 - self.split_factor) + end * self.split_factor,
         if low <= self.max_length {Some(low)} else {None},
         if high <= self.max_length {Some(high)} else {None}
        )
    }
}
...
{
    let curve = Box::new(TestCurve{split_factor: 0.7, max_length: 0.001});
