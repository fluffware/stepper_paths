...
impl StepperContext {
    pub fn new(
        a_max: &[i32; N_CHANNELS],
        v_max: &[i32; N_CHANNELS],
        step_scale: &[f64; N_CHANNELS],
    ) -> StepperContext {
    ...
    pub fn add_acc_interval(&mut self, acc_x: i16, acc_y: i16, interval: u32) {
        ...
        for (i, ref mut ch) in self.channels.iter_mut().enumerate() {
            ...
            ch.pos += 2 * interval as i64 * ch.v as i64
    ...
    fn acc_segs_to_events(&mut self, acc: &[Vec<AccSegment>]) {
        ...
        loop {
            ...
            for (i, ch) in self.channels.iter().enumerate() {
                if ch.ticks < min && p[i].peek().is_some() {
                    min_index = Some(i);
                    min = ch.ticks;
                }
            }
    ...
    fn to_steps(&self, coord: f64, channel: usize) -> i64 {
        ...
        assert!(
            s < (i64::max_value() as f64) && s > (i64::min_value() as f64),
    ...
    fn to_step_v(&self, v: f64, channel: usize) -> i32 {
        ...
        assert!(
            s < (i32::max_value() as f64) && s > (i32::min_value() as f64),
            "Step coordinate out of range"
        );
        s as i32
    }

    fn coord_from_steps(&self, steps: i64, channel: usize) -> f64 {
    ...
    pub fn step_goto_speed(&mut self, x: i64, y: i64, vx: i32, vy: i32) {
        ...
        let limits = &[
            PathLimits {
                ds: x - self.channels[X_INDEX].pos,
                v0: self.channels[X_INDEX].v,
                vn: vx,
                a: self.step_a_max[X_INDEX],
                vmax: self.step_v_max[X_INDEX],
            },
            PathLimits {
                ds: y - self.channels[Y_INDEX].pos,
                v0: self.channels[Y_INDEX].v,
                vn: vy,
    ...
    pub fn goto_speed(&mut self, x: f64, y: f64, vx: f64, vy: f64) {
        self.step_goto_speed(
            self.to_steps(x, X_INDEX),
    ...
    pub fn goto(&mut self, x: f64, y: f64) {
        self.step_goto_speed(self.to_steps(x, X_INDEX), self.to_steps(y, Y_INDEX), 0, 0)
    }
    ...
    pub fn speed(&mut self, v_x: f64, v_y: f64) {
        ...
        for i in 0..N_CHANNELS {
            let (t, dt) = (v[i] - self.channels[i].v).div_rem(&self.step_a_max[i]);
            let a = if v[i] >= self.channels[i].v {
                self.step_a_max[i]
            } else {
                -self.step_a_max[i]
            };
            acc[i].push(AccSegment {
                interval: t.unsigned_abs() as u16,
                acc: a as i16,
            });
            self.channels[i].pos +=
                ((2 * self.channels[i].v + self.step_a_max[i] * t) * t.abs()) as i64;
            if dt != 0 {
                acc[i].push(AccSegment {
                    interval: 1,
                    acc: dt as i16,
                });
                self.channels[i].pos += (self.channels[i].v + self.step_a_max[i] * t + v[i]) as i64;
            }
    ...
    pub fn approx_curve(&mut self, start: Point, curve: &dyn CurveInfo, v: f64) -> [i64; 2] {
        ...
        for t in 1..=(steps as i32) {
            ...
            for d in 0..N_CHANNELS {
                ...
                let a_pos = limit(
                    (step_pos - (self.channels[d].pos + 2 * self.channels[d].v as i64) as f64)
                        / 2.0,
                    -a_max[d],
                    a_max[d],
                );
                ...
                if (pos_err_acc[d] > 0.0 && a_pos < 0.0) || (pos_err_acc[d] < 0.0 && a_pos > 0.0) {
                    pos_err_acc[d] = 0.0;
                }

                // Acceleration for correct speed
                let a_v = limit(step_v - self.channels[d].v as f64, -a_max[d], a_max[d]);

                // println!("a_pos: {} a_v: {}", a_pos, a_v);
                acc[d] = (a_v * (1.0 - weigth) + (a_pos + pos_err_acc[d]) * weigth).round() as i16;

                // Limit speed
                if acc[d] < 0 && self.channels[d].v + (acc[d] as i32) < -v_max[d] {
                    acc[d] = (-v_max[d] - self.channels[d].v) as i16;
                } else if acc[d] > 0 && self.channels[d].v + (acc[d] as i32) > v_max[d] {
                    acc[d] = (v_max[d] - self.channels[d].v) as i16;
                }
            }
    ...
    fn draw_curve(
        &mut self,
        start: Point,
        curve: &mut curves::concat_curve::ConcatCurve,
        v: f64,
        ...
        if !curve.is_empty() {
            ...
            self.add_weight_change(self.curve_weight, self.channels[X_INDEX].ticks);
    ...
    pub fn draw_curves(
        ...
        let mut seg = match iter.next() {
            Some(s) => s,
            None => return Ok(max_err),
        };
        let mut current_pos = Point {
            x: self.coord_from_steps(self.channels[X_INDEX].pos, X_INDEX),
            y: self.coord_from_steps(self.channels[Y_INDEX].pos, Y_INDEX),
        };
    ...
    pub fn reset(&mut self) {
        self.events.clear();
        for i in 0..N_CHANNELS {
            self.channels[i] = ChannelContext {
                pos: 0,
                v: 0,
                ticks: 0,
            };
...
fn test_approx_curve_line() {
    let mut ctxt = StepperContext::new(&[10, 10], &[10, 10], &[3.0, 7.0]);
