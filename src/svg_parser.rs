...
pub type ElementFilter = dyn Fn(&OwnedName, &Vec<OwnedAttribute>) -> bool;
pub fn parse_document<T: Read>(
    input: T,
    t0: &Transform,
    filter: Box<ElementFilter>,
            ...
            Ok(XmlEvent::StartElement {
                ...
                if name.namespace == Some(SVG_NS.to_string()) {
                    ...
                    if name.local_name == "path" {
                        match d {
                            None => return Err("path element has no d attribute".into()),
                            Some(d) => {
