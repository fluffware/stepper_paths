...
impl<T:Debug+PartialOrd> Debug for Range<T> {
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        return write!(f,"[{:?} - {:?}]", self.low, self.high);
    }
}
    ...
    pub fn and(&self, low: T, high: T) -> Self
    {
        ...
        while let Some(r) = i.next() {
            if r.high > low {
                let mut rn = r;
                while rn.low < high {
                    res.ranges.push(Range {low: max(low, rn.low),
...
fn or_test()
{
    ...
    r.or(8, 9);
    assert_eq!(r,*MultiRange::<i32>::new().or_slice(&[(2,7),(8,9), (10,34)]));
    r.or(7,8);
...
#[cfg(test)]
fn check_and<T:PartialOrd+Copy+Debug>(a:&MultiRange<T>, b:&MultiRange<T>, 
                           res: &MultiRange<T>) 
{
...
fn and_test()
{
    ...
    check_and(&r,&new_from_slice(&[(5,11)]), &new_from_slice(&[(10,11)]));
    check_and(&r,&new_from_slice(&[(4,10)]), &new_from_slice(&[(4,5)]));
    check_and(&r,&new_from_slice(&[(0,4)]), &new_from_slice(&[(2,4)]));
    check_and(&r,&new_from_slice(&[(20,40)]), &new_from_slice(&[(20,23)]));
