...
impl Bezier {
    pub fn new(c1: Point, c2: Point, p2: Point) -> Bezier {
        ...
        for s in splits {
            ...
            let p1 = split_segment(&mut seg, split_pos, prev_split);
    ...
    pub fn bezier_value(&self, t: f64) -> (Point, Vector) {
        let deriv = Vector {
            x: bezier_deriv(&self.dx, t),
            y: bezier_deriv(&self.dy, t),
        };
        let scale = 1.0 / deriv.length();
        (
            Point {
                x: bezier_poly_value(&self.cx, t),
...
fn split_bezier_coord(c1: f64, c2: f64, p2: f64, pos: f64) -> (f64, f64, f64, f64, f64) {
    let q0 = c1 * pos;
    let q1 = c1 + (c2 - c1) * pos;
    let q2 = c2 + (p2 - c2) * pos;
    let r0 = q0 + (q1 - q0) * pos;
...
fn split_segment(p2: &mut BezierSegment, pos: f64, t_start: f64) -> BezierSegment {
    // split x
    let (c11x, c12x, p1x, c21x, c22x) = split_bezier_coord(p2.c1.x, p2.c2.x, p2.p, pos);
    // split y
    let (c11y, c12y, p1y, c21y, c22y) = split_bezier_coord(p2.c1.y, p2.c2.y, 0.0, pos);
    p2.c1 = Point {
        x: c21x - p1x,
        y: c21y - p1y,
    };
...
fn flatten_segments(curved: &mut Vec<BezierSegment>, precision: f64) -> Vec<BezierSegment> {
...
impl CurveInfo for Bezier {
    ...
    fn value(&self, pos: f64) -> (Point, Vector) {
        let t = if pos <= 0.0 {
            ...
            match self
                .segments
                .binary_search_by(|s| s.p.partial_cmp(&pos).unwrap())
            {
                ...
                Err(i) => {
                    ...
                    match solve_2nd_degree(&[3.0 * a3, 2.0 * a2, a1]) {
                        ...
                        Solution2ndDegree::One(x1) if !(0.0..=1.0).contains(&x1) => {}
...
fn solve_2nd_degree(c: &[f64; 3]) -> Solution2ndDegree {
    ...
    if w == 0.0 {
        return Solution2ndDegree::One(-p);
    }
    let sqrt_w = w.sqrt();
