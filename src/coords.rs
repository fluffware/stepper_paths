...
impl Vector {
    ...
    pub fn angle_cos(&self, other: &Vector) -> f64 {
        (self.x * other.x + self.y * other.y)
            / ((self.x * self.x + self.y * self.y) * (other.x * other.x + other.y * other.y)).sqrt()
    }
}
...
impl Transform {
    ...
    pub fn skew_x(a: f64) -> Transform {
        Transform {
            matrix: [1.0, 0.0, a.tan(), 1.0, 0.0, 0.0],
        }
    }

    pub fn skew_y(a: f64) -> Transform {
        Transform {
            matrix: [1.0, a.tan(), 0.0, 1.0, 0.0, 0.0],
        }
    }
...
impl std::ops::Mul for Transform {
    ...
    fn mul(self, t: Self) -> Self {
        Transform {
            matrix: matrix_mul(&self.matrix, &t.matrix),
        }
    }
}
...
impl std::ops::Mul<Vector> for Transform {
    ...
    fn mul(self, v: Vector) -> Vector {
        Vector {
            x: self.matrix[0] * v.x + self.matrix[2] * v.y + self.matrix[4],
...
fn test_transform() {
    ...
    assert_transform_eq(
        &(a * b),
        &Transform::new(&[3.29, 31.33, -400.2, 312.9, -41.8, 111.85]),
    );
    assert_transform_eq(
        &(b * a),
        &Transform::new(&[168.89, 132.9, 85.1, 147.3, -271.365, -234.645]),
    );
}
...
fn test_diff_sign() {
    assert_eq!(
        Vector { x: 1.0, y: 1.0 }.diff_sign(&Vector { x: -4.0, y: -4.0 }),
        0.0
    );
...
fn check_decompose(tr: Vector, scale: Vector, rot: f64, skew_x: f64, skew_y: f64) {
    ...
    println!(
        "in:  translate({}) scale({}) rotate({}) skewX({}) skewY({})",
        tr, scale, rot, skew_x, skew_y
    );
    let (tr, scale, rot, skew_x, skew_y) = b.decompose();

    println!(
        "res: translate({}) scale({}) rotate({}) skewX({}) skewY({})",
        tr, scale, rot, skew_x, skew_y
    );
...
fn test_decompose() {
    ...
    let b = Transform {
        matrix: [0.0, 1.0, 0.0, 2.0, -1.5, 8.0],
    };
    let (tr, scale, rot, skew_x, skew_y) = b.decompose();
    let c = Transform::translate(tr.x, tr.y)
        * Transform::rotate(rot)
